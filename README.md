...
## 功能
每日北京时间凌晨6点（即UTC+8 6:00，对应UTC 22:00）执行定时签到（程序会自动添加随机延时），获取额外的流量奖励。
...
## 部署过程
1. 右上角Fork此仓库
2. 然后到`Settings`->`Secrets and variables`->`Actions`->`Secrets`->`Repository secrets`->`New repository secret`，新建以下环境变量：
3. 在`Actions`的所有工作流中找到`Common Airport Checkin`，手动触发运行一次`Run workflow`，以后每天项目都会自动运行。
